...
class OchiaiStructuredAnalyzer:
    ...
    def _create_structured_prompt(self, text: str, paper_metadata: PaperMetadata, query: str) -> str:
        ...
        # Define the exact JSON structure with examples
        json_template = {
            "what_is_it": "この研究は[具体的な内容]に関するもので、[主要な問題]を解決することを目的としている",
            "comparison_with_prior_work": "従来の[手法名]と比較して、本研究は[具体的な違い]という点で新規性がある",
            "key_technique": "提案手法の核心は[技術名]であり、[具体的な説明]によって[効果]を実現している",
            "validation_method": "実験では[データセット名]を用いて、[評価指標]により検証を行った",
            "experimental_results": "[具体的な数値]の改善を達成し、[ベースライン]と比較して[パーセント]の性能向上を示した",
            "discussion_points": "本研究の限界として[具体的な制限]があり、今後の課題として[発展の方向性]が考えられる",
            "implementation_details": "実装には[フレームワーク/言語]を使用し、[具体的な工夫]を行った",
            "why_selected": f"検索クエリ「{query}」に対して、[具体的な関連性]の観点から選ばれた",
            "applicability": "この技術は[応用分野]に適用可能であり、[具体的な用途]が期待される",
            "next_papers": ["[関連論文1のタイトル]", "[関連論文2のタイトル]"]
        }
...
2. 各フィールドは具体的な内容を含む完全な文章にしてください
3. [括弧内]の部分を実際の内容に置き換えてください
4. next_papersは具体的な論文タイトルのリストにしてください（最低2つ）
5. JSONの前後に余分なテキストを含めないでください
